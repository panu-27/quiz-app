...
function FullSettingsView({ onBack, name, email, avatarUrl, onAvatarChange }) {
 ...
 function MobileSettingsPage({ onBack, name, email, avatarUrl, onAvatarChange }) {
   ...
   return (
     <div className={`min-h-screen flex flex-col ${isDark ? 'bg-[#000711]' : 'bg-[#F4F7FC]'}`}>
       {/* Header */}
       <div
         className={`sticky top-0 z-50 transition-all duration-300 ${isDark
             ? 'bg-[#171F2A] rounded-b-lg'
       ...
       >
         ...
         <div className="px-5 py-4 flex items-center gap-4">
           ...
           <h1 className={`text-lg -ml-2 font-bold tracking-tight font-display ${isDark ? 'text-white' : 'text-slate-900'}`}>Profile & settings</h1>
         </div>
 
         {/* Goal Box - merged into header layout */}
         <div className="px-5 pb-5 pt-1 flex items-center justify-between">
           <button onClick={() => setIsGoalExpanded(!isGoalExpanded)} className="flex items-center gap-3 text-left focus:outline-none">
             <div className="w-12 h-12 flex items-center justify-center shrink-0 bg-transparent">
               {getGoalIcon(selectedGoal)}
             </div>
             <div>
               <div className="flex items-center gap-1">
                 <h3 className={`text-[15px] font-bold ${isDark ? 'text-white' : 'text-slate-900'} font-display`}>{selectedGoal}</h3>
                 <ChevronDown size={14} className={`text-slate-450 mt-0.5 transition-transform duration-300 ${isGoalExpanded ? 'rotate-180' : ''}`} />
               </div>
               {user?.approved === false && (
                 <p className={`text-[12px] ${isDark ? 'text-slate-400' : 'text-slate-500'} font-medium mt-0.5`}>Free plan</p>
               )}
             </div>
           </button>
           <button onClick={() => setIsGoalExpanded(!isGoalExpanded)} className={`px-4 py-1.5 text-[12px] font-bold rounded-lg border bg-transparent ${isDark ? 'border-slate-800 text-white hover:bg-slate-800/45' : 'border-slate-300 text-slate-800 hover:bg-slate-105/55'} active:scale-95 transition-all`}>
             Manage
           </button>
         </div>
         ...
         <div
           className={`overflow-hidden transition-all duration-300 ease-in-out flex flex-col gap-0 border-t ${isDark ? 'border-slate-800/60' : 'border-slate-100'
             } ${isGoalExpanded ? 'max-h-[200px] opacity-100 py-1' : 'max-h-0 opacity-0 pointer-events-none'}`}
         >
           {['Boards', 'MHT-CET'].map(goal => {
             ...
             return (
               <button
                 ...
                 className={`
                   w-full py-3 px-5 flex items-center justify-between transition-colors text-left
                   ${isDark ? 'hover:bg-slate-800/40 text-white' : 'hover:bg-slate-50 text-slate-800'}
                   ${isSelected ? 'font-bold' : 'font-semibold'}
                 `}
               >
                 <div className="flex items-center gap-3">
                   <div className={`w-4 h-4 rounded-full border-[2px] flex items-center justify-center transition-colors
                     ${isSelected
                       ? 'border-[#7A41F7]'
                       : (isDark ? 'border-slate-700' : 'border-slate-300')}
                   `}>
                     {isSelected && (
                       <div className="w-2 h-2 rounded-full bg-[#7A41F7]" />
                     )}
                   </div>
                   <span className="text-sm font-display">{goal}</span>
                 </div>
               </button>
             );
           })}
       ...
       <div className={`flex-1 overflow-y-auto px-2 py-3 space-y-2.5 ${isDark ? 'bg-[#000711]' : 'bg-[#F4F7FC]'}`}>
 
         {/* Store Box */}
         <div className={`rounded-lg p-3.5 flex items-center justify-between border ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'} shadow-sm`}>
           <div className="flex-1">
             <h3 className={`text-[15px] font-bold ${isDark ? 'text-white' : 'text-slate-900'} font-display mb-1`}>Unacademy Store</h3>
             <p className={`text-[12px] ${isDark ? 'text-slate-400' : 'text-slate-500'} leading-relaxed pr-4`}>Exclusive batches, Previous year question papers, Test series.</p>
           </div>
           <div className={`w-12 h-12 ${isDark ? 'bg-[#000711]/60' : 'bg-emerald-50'} rounded-sm flex items-center justify-center shrink-0`}>
             <ShoppingBag className="text-emerald-500" size={24} />
           </div>
         </div>
 
         {/* Profile Info Box */}
         <div className={`rounded-lg px-4 py-4.5 border ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'} shadow-sm flex items-center gap-4`}>
           <div className={`w-16 h-16 rounded-sm overflow-hidden border-2 border-emerald-500 shrink-0 relative flex items-center justify-center ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}>
             {hasCustomAvatar(avatarUrl) ? (
               <img src={avatarUrl} alt="avatar" className="w-full h-full object-cover" />
             ) : (
               <GraduationCap className="text-emerald-500 w-10 h-10" />
             )}
             <div className="absolute -bottom-1 -right-1 bg-amber-400 w-5 h-5 rounded-md rotate-45 flex items-center justify-center border border-white">
               <div className="-rotate-45 w-2 h-2 rounded-full bg-white opacity-80" />
             </div>
           </div>
           <div>
             <h2 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'} font-display`}>{name}</h2>
             <p className="text-[13px] font-bold text-amber-500 mt-0.5">Beginner • 345</p>
             <div className={`mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-sm border ${isDark ? 'bg-[#000711] border-slate-800/80' : 'bg-slate-50 border-slate-200'}`}>
               <Medal size={12} className={isDark ? 'text-slate-400' : 'text-slate-400'} />
               <span className={`text-[11px] font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Rank 2,16,845 in IIT JEE</span>
             </div>
           </div>
         </div>
 
         {/* My Library Section */}
         <div className={`rounded-lg px-4 py-4.5 border ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'} shadow-sm`}>
           <h3 className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'} font-display mb-4`}>My library</h3>
           <div className="grid grid-cols-2 gap-2">
             {[
               { icon: <PlaySquare size={16} className="text-emerald-500" />, label: "Enrollments" },
               { icon: <Download size={16} className="text-purple-500" />, label: "Downloads" },
               { icon: <Bell size={16} className="text-emerald-500" />, label: "Updates" },
               { icon: <Users size={16} className="text-blue-500" />, label: "My educators" },
               { icon: <HelpCircle size={16} className="text-sky-500" />, label: "FAQs" },
               { icon: <Settings size={16} className="text-slate-400" />, label: "Settings", action: () => setShowFullSettings(true) }
             ].map((item, i) => (
               <button key={i} onClick={item.action} className={`flex items-center gap-2 px-2.5 py-3 rounded-lg border bg-transparent ${isDark ? 'border-slate-800/80 hover:bg-slate-800/20' : 'border-slate-200 hover:bg-slate-50/50'} active:scale-95 transition-all text-left`}>
                 {item.icon}
                 <span className={`text-[13px] font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>{item.label}</span>
               </button>
             ))}
           </div>
         </div>
         ...
         {/* Activity Section */}
         <div className={`rounded-lg px-4 py-4.5 border ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'} shadow-sm`}>
           <h3 className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'} font-display mb-1`}>Activity</h3>
           <p className={`text-[12px] ${isDark ? 'text-slate-400' : 'text-slate-500'} font-medium mb-4`}>Daily stats on how much you learn & practice</p>
           <div className={`flex items-center justify-center py-6 border-t border-dashed ${isDark ? 'border-slate-800/60' : 'border-slate-200'}`}>
             <span className={`text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>No activity yet. Start learning!</span>
           </div>
         </div>
       </div>
     </div>
 ...
 export default function StudentProfile() {
   ...
   return (
     <>
       ...
       ══════════════════════════════════════════ */}
       <div className={`lg:hidden min-h-screen flex flex-col ${isDark ? 'bg-[#000711]' : 'bg-[#F4F7FC]'}`}>
 
         {/* Sticky Header containing Back button + Goal Info */}
         <div
           className={`sticky top-0 z-50 transition-all duration-300 ${isDark
               ? 'bg-[#171F2A] rounded-b-lg'
               : 'bg-white border-b border-slate-200'
         ...
         >
           ...
           <div className="flex items-center gap-4 px-5 py-4">
             ...
             <h1 className={`text-[18px] font-bold tracking-tight font-display ${isDark ? 'text-white' : 'text-slate-900'}`}>Profile &amp; settings</h1>
           </div>
           ...
           <div className="px-5 pb-5 pt-1 flex items-center justify-between">
             <button onClick={() => setIsGoalExpanded(!isGoalExpanded)} className="flex items-center gap-3 text-left focus:outline-none">
               ...
               <div>
                 <div className="flex items-center gap-1">
                   <span className={`text-[16px] font-extrabold font-display ${isDark ? 'text-white' : 'text-slate-900'}`}>
                     {selectedGoal}
                   </span>
                   <ChevronDown size={14} className={`text-slate-440 mt-0.5 transition-transform duration-300 ${isGoalExpanded ? 'rotate-180' : ''}`} />
                 </div>
                 {authUser?.approved === false && (
                   <p className={`text-[12px] font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Free plan</p>
                 )}
               </div>
             </button>
             <button
               onClick={() => setIsGoalExpanded(!isGoalExpanded)}
               className={`px-4 py-2 text-[13px] font-bold rounded-lg border bg-transparent transition-all active:scale-95 ${isDark
                   ? 'border-slate-800 text-white hover:bg-slate-850/45'
                   : 'border-slate-300 text-slate-800 hover:bg-slate-105/55'
                 }`}
             >
               Manage
             </button>
         ...
         {/* Scrollable Content */}
         <div className={`flex-1 overflow-y-auto px-2 pt-3 pb-28 space-y-2.5 ${isDark ? 'bg-[#000711]' : 'bg-[#F4F7FC]'}`}>
           {/* Store Card */}
           <div className={`rounded-lg px-4 py-4.5 flex items-center justify-between border shadow-sm ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'}`}>
             <div className="flex-1 pr-4">
               <h3 className={`text-[16px] font-extrabold font-display mb-1.5 ${isDark ? 'text-white' : 'text-slate-900'}`}>Store</h3>
               <p className={`text-[12px] leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Exclusive batches, Previous year question papers, Test series.</p>
             </div>
             <div className={`w-12 h-12 rounded-sm flex items-center justify-center shrink-0 ${isDark ? 'bg-[#000711]/60' : 'bg-teal-50'}`}>
               <ShoppingBag size={24} className="text-teal-500" />
             </div>
           </div>
 
           {/* Profile Card */}
           <div className={`rounded-lg px-4 py-4.5 flex items-center gap-4 border shadow-sm ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'}`}>
             {/* Avatar with badge */}
             <div className="relative shrink-0">
               <div className={`w-[72px] h-[72px] rounded-sm overflow-hidden border-[2.5px] border-emerald-500 flex items-center justify-center ${isDark ? 'bg-slate-800' : 'bg-slate-700'}`}>
                 {loading
               ...
               {/* Gold diamond badge */}
               <div className={`absolute -bottom-1.5 -right-1.5 w-[22px] h-[22px] bg-amber-400 rounded-[5px] rotate-45 border-2 shadow-md flex items-center justify-center ${isDark ? 'border-[#171F2A]' : 'border-white'}`}>
                 <div className="-rotate-45 w-2.5 h-2.5 bg-white/70 rounded-sm" />
               </div>
             </div>
             <div className="flex-1 min-w-0">
               <h2 className={`text-[18px] font-extrabold font-display leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{name}</h2>
               <p className="text-[13px] font-bold text-amber-500 mt-0.5">Beginner &bull; 345</p>
               <div className={`mt-2.5 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-sm border ${isDark ? 'bg-[#000711] border-slate-800/80' : 'bg-slate-50 border-slate-200'}`}>
                 <Medal size={11} className={isDark ? 'text-slate-400' : 'text-slate-550'} />
                 <span className={`text-[11px] font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Rank 2,16,845 in {localStorage.getItem('selectedGoal') || 'IIT JEE'}</span>
               </div>
             </div>
           </div>
 
           {/* My Library */}
           <div className={`rounded-lg px-4 py-4.5 border shadow-sm ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'}`}>
             <h3 className={`text-[16px] font-extrabold font-display mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`}>My library</h3>
             <div className="grid grid-cols-2 gap-2">
               {[
                 { icon: <PlaySquare size={18} className="text-emerald-500" />, label: 'Enrollments' },
                 { icon: <Download size={18} className="text-purple-500" />, label: 'Downloads' },
                 { icon: <Bell size={18} className="text-teal-500" />, label: 'Updates' },
                 { icon: <Users size={18} className="text-blue-500" />, label: 'My educators' },
                 { icon: <HelpCircle size={18} className="text-sky-500" />, label: 'FAQs' },
                 { icon: <Settings size={18} className={isDark ? 'text-slate-400' : 'text-slate-500'} />, label: 'Settings', action: () => setMobileSettings(true) },
               ].map((item, i) => (
                 <button
                   key={i}
                   onClick={item.action}
                   className={`flex items-center gap-3 px-4 py-3.5 rounded-lg border bg-transparent text-left active:scale-95 transition-all ${isDark ? 'border-slate-800/80 hover:bg-slate-800/20' : 'border-slate-200 hover:bg-slate-50/50'}`}
                 >
                   {item.icon}
                   <span className={`text-[13px] font-bold leading-tight ${isDark ? 'text-white' : 'text-slate-800'}`}>{item.label}</span>
                 </button>
               ))}
             </div>
           </div>
           ...
           {/* Activity */}
           <div className={`rounded-lg px-5 py-5 border shadow-sm ${isDark ? 'bg-[#171F2A] border-transparent' : 'bg-white border-slate-200'}`}>
             <h3 className={`text-[16px] font-extrabold font-display mb-0.5 ${isDark ? 'text-white' : 'text-slate-900'}`}>Activity</h3>
             <p className={`text-[12px] font-medium mb-5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
               Daily stats on how much you learn &amp; practice
             </p>
             <div className={`flex items-center justify-center py-8 border-t border-dashed ${isDark ? 'border-slate-800/60' : 'border-slate-200'}`}>
               <span className={`text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>No activity yet — start learning!</span>
             </div>
           </div>
         </div>
