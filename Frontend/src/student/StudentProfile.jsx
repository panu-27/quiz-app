...
function Toast({ msg, type, onDone }) {
    ...
    return (
        <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 z-[500] flex items-center gap-2.5 px-5 py-3 rounded-sm text-sm font-semibold text-white fade-up pointer-events-none font-body
...
function AvatarPanel({ name, avatarUrl, onAvatarChange, onClose }) {
    ...
    return (
        <div className="slide-down panel-open bg-white border border-slate-100 rounded-3xl p-6 mt-2">
    ...
    return (
        <div className={`min-h-screen flex flex-col ${isDark ? 'bg-[#000711]' : 'bg-[#F6F8FC]'}`}>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6 pb-16">
                ...
                <section className={`rounded-3xl p-5 border ${isDark ? 'bg-[#171F2A] border-slate-800/85' : 'bg-white border-slate-100'}`}>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-4 font-body">Profile Photo</p>
                    <div className="flex items-center gap-4 mb-4">
                        <div className="relative shrink-0">
                            <div className={`w-20 h-20 rounded-full overflow-hidden border-2 bg-slate-50 ${isDark ? 'border-slate-850' : 'border-slate-100'}`}>
                ...
                <section className={`rounded-3xl p-5 border ${isDark ? 'bg-[#171F2A] border-slate-800/85' : 'bg-white border-slate-100'} space-y-3`}>
                ...
                {/* Logout Section */}
                <section className={`rounded-lg p-5 border ${isDark ? 'bg-[#171F2A] border-slate-800/85' : 'bg-white border-slate-100'}`}>
    ...
    return (
        <div className={`min-h-screen flex flex-col ${isDark ? 'bg-[#0A0F1A]' : 'bg-[#F4F7FC]'}`}>
            ...
            <div className={`flex-1 overflow-y-auto px-2 pt-2 pb-28 space-y-3 ${isDark ? 'bg-[#0A0F1A]' : 'bg-[#F4F7FC]'}`}>
                ...
                {/* Profile Info Row (No Background) */}
                <div className="flex items-center gap-4 px-2 py-2">
                    {/* Simple Avatar */}
                    <div className={`w-[72px] h-[72px] shrink-0 rounded-full overflow-hidden flex items-center justify-center ${isDark ? 'bg-slate-800' : 'bg-slate-200'}`}>
                        {hasCustomAvatar(avatarUrl) ? (
                            <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" onError={e => { e.target.src = DEFAULT_AVATAR(name); }} />
                        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                        <h2 className={`text-[20px] font-bold font-display leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{name}</h2>
                ...
                <div className={`rounded-xl p-4 ${isDark ? 'bg-[#151E2E]' : 'bg-white border border-slate-200'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            ...
                            { icon: <Award size={18} className="text-amber-500" />, label: "Leaderboard", action: () => { window.location.href = '/student/personal'; } },
...
export default function StudentProfile() {
    ...
    const FALLBACK_ALL_TIME = [
        ...
        { rank: 2, name: "Jashan", points: 7334815, level: 33, percentile: 100, avatar: null },
        { rank: 3, name: "Bk jha", points: 6680144, level: 33, percentile: 100, avatar: null },
        { rank: 4, name: "Madelyn Dias", points: 45590, level: 22, percentile: 90, avatar: null }
    ];
...
══════════════════════════════════════════ */}
            <div className="hidden lg:flex flex-col min-h-screen bg-[#F6F8FC]">
                ...
                <div className="flex-1 max-w-[1440px] mx-auto w-full px-8 xl:px-12 py-10 flex gap-8 items-start">

                    {/* ── LEFT PURPLE CARD (sticky) ── */}
                    <div className="w-[320px] xl:w-[360px] shrink-0 bg-[#7A41F7] rounded-lg flex flex-col relative overflow-hidden -purple-200/40 self-start sticky top-[88px]">
                        ...
                        <div className="flex justify-between items-center px-6 pt-6 pb-2 relative z-10">
                            <button onClick={() => navigate(-1)} className="p-0.5 bg-transparent border-none text-white active:scale-95 transition-all">
                        ...
                        {loading ? <DesktopLeftSkeleton /> : (
                            <div className="flex flex-col items-center px-6 pt-4 pb-8 relative z-10 fade-up">
                                <div className="relative mb-4">
                                    ...
                                    <div className="relative w-24 h-24 bg-white rounded-full border-[5px] border-white overflow-hidden flex items-center justify-center">
                                    ...
                                    <button
                                        onClick={() => togglePanel("avatar")}
                                        className="absolute -bottom-1 -right-1 bg-slate-900/90 text-amber-400 w-8 h-8 rounded-lg flex items-center justify-center border-2 border-white hover:bg-slate-800 transition-colors"
                                    >
                                        <Camera size={13} />
                                    </button>
                                </div>
                            ...
                            <div className="fade-up space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-3 gap-5 mb-5">
                                        ...
                                        <div className="bg-[#EBFDEB] rounded-lg p-6 col-span-1">
                                            <div className="flex gap-2 mb-5">
                                ...
                                <div>
                                    ...
                                    <div>
                                        <button
                                            onClick={() => togglePanel("avatar")}
                                            className="w-full bg-transparent border border-slate-200 rounded-sm p-5 flex items-center gap-4 hover: hover:-translate-y-0.5 transition-all text-left group"
                                        >
...
══════════════════════════════════════════ */}
            <div className={`lg:hidden  min-h-screen flex  flex-col ${isDark ? 'bg-[#0A0F1A]' : 'bg-[#F4F7FC]'}`}>
                ...
                {/* Sticky Header containing Back button + Profile */}
                <div className={`flex-shrink-0 sticky top-0 z-40 px-5 pb-3 flex rounded-b-2xl flex-col gap-4 ${isDark ? 'bg-[#151E2E] border-b border-slate-800' : 'bg-white border-b border-slate-200'}`} style={{ paddingTop: STATUS_BAR_H + 8 }}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <button
                                onClick={() => navigate(-1)}
                                className={`w-10 h-10 -ml-2 flex items-center justify-center transition-colors bg-transparent border-none p-0 ${isDark ? 'text-white hover:bg-slate-800/50' : 'text-slate-800 hover:bg-slate-200/50'} rounded-full`}
                            >
                                <ArrowLeft size={24} />
                            </button>
                            <h1 className={`text-[17px] font-bold truncate leading-tight m-0 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                                Profile
                            </h1>
                        </div>
        ...
        `}
            ...
            `}
                    ...
                    {/* Profile Info Row (Attached Below in Sticky Header) */}
                        ...
                        >
                            {loading ? (
                                ...
                                <img
                                    ...
                                    className="w-full h-full object-cover"
                                    onError={e => {
                                        e.target.src = DEFAULT_AVATAR(name);
                                    }}
                                />
                            ) : (
                ...
                <div className={`flex-1 overflow-y-auto px-2 pt-2 pb-28 space-y-3 ${isDark ? 'bg-[#0A0F1A]' : 'bg-[#F4F7FC]'}`}>
                    ...
                    <div className={`rounded-xl p-4 ${isDark ? 'bg-[#151E2E]' : 'bg-white border border-slate-200'}`}>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className={`text-[17px] font-bold font-display ${isDark ? 'text-white' : 'text-slate-900'}`}>Stats</h3>
                            <span className={`text-[11px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>ALL TIME</span>
                        </div>

                        <div className="grid grid-cols-2 gap-3">
                            {/* Card 1: Percentile */}
                            <div className={`p-4 rounded-xl relative overflow-hidden ${isDark ? 'bg-[#0A0F1A]' : 'bg-slate-50'}`}>
                                <div className={`text-[10px] font-bold uppercase tracking-wider mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>PREDICTED PERCENTILE</div>
                                <div className={`text-[22px] font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>98.5</div>
                                <div className="absolute -bottom-2 -right-2 w-12 h-12 rounded-full bg-[#DBEAFE] flex items-center justify-center">
                                    <Globe size={18} className="text-[#3B82F6] -mt-2 -ml-2" />
                                </div>
                            </div>

                            {/* Card 2: Class Rank */}
                            <div className={`p-4 rounded-xl relative overflow-hidden ${isDark ? 'bg-[#0A0F1A]' : 'bg-slate-50'}`}>
                                <div className={`text-[10px] font-bold uppercase tracking-wider mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>CLASS RANK</div>
                                <div className={`text-[22px] font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>12</div>
                                <div className="absolute -bottom-2 -right-2 w-12 h-12 rounded-full bg-slate-200 flex items-center justify-center">
                                    <MapPin size={18} className="text-slate-600 -mt-2 -ml-2" />
                                </div>
                            </div>

                            {/* Card 3: State Rank */}
                            <div className={`p-4 rounded-xl relative overflow-hidden ${isDark ? 'bg-[#0A0F1A]' : 'bg-slate-50'}`}>
                                <div className={`text-[10px] font-bold uppercase tracking-wider mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>STATE RANK</div>
                                <div className={`text-[22px] font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>1,420</div>
                                <div className="absolute -bottom-2 -right-2 w-12 h-12 rounded-full bg-[#D1FAE5] flex items-center justify-center">
                                    <Star size={18} className="text-[#10B981] -mt-2 -ml-2" />
                                </div>
                            </div>

                            {/* Card 4: Accuracy */}
                            <div className={`p-4 rounded-xl relative overflow-hidden ${isDark ? 'bg-[#0A0F1A]' : 'bg-slate-50'}`}>
                                <div className={`text-[10px] font-bold uppercase tracking-wider mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>ACCURACY</div>
                                <div className={`text-[22px] font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>92%</div>
                                <div className="absolute -bottom-2 -right-2 w-12 h-12 rounded-full bg-[#E0F2FE] flex items-center justify-center">
                                    <Target size={18} className="text-[#0EA5E9] -mt-2 -ml-2" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={`rounded-xl p-4 ${isDark ? 'bg-[#151E2E]' : 'bg-white border border-slate-200'}`}>
                        ...
                        <div className={`rounded-xl border flex divide-x mb-6 ${isDark ? 'border-[#2A3649] divide-[#2A3649]' : 'border-slate-200 divide-slate-200'}`}>
                            <div className="flex-1 p-4 flex flex-col gap-2">
                                <span className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>CURRENT</span>
                                <div className="flex items-center gap-2">
                        ...
                        <div className="space-y-4">
                            {/* Days Header */}
                            <div className="flex items-center justify-between pl-[4.5rem] pr-2">
                                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                                    <span key={i} className={`text-[11px] font-bold w-[22px] text-center ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{d}</span>
                                ))}
                            </div>
...
const MetricProgress = ({ label, value, color, bgColor, icon, desktop = false }) => (
    <div className={`${bgColor} p-5 rounded-lg border flex flex-col items-start gap-4 hover: transition-all`}>
        <div className="flex items-center gap-2">
            <div className={`p-2 rounded-sm ${desktop ? "bg-white/10" : "bg-white/60"} ${color}`}>{icon}</div>
        ...
        <div className="flex items-end justify-between w-full">
            <div className="relative w-12 h-12 flex items-center justify-center shrink-0">
                <svg className="w-full h-full -rotate-90 drop-">
...
const InfoRow = ({ label, value, status, icon, theme, isMentorList, mentors }) => {
    ...
    return (
        <div className="flex items-start justify-between bg-white p-4 py-5 rounded-[2rem] border border-slate-50 hover: transition-all group">
            <div className="flex items-start gap-4 min-w-0 flex-1">
                <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-100 shrink-0 group-hover:bg-white transition-colors mt-1">{icon}</div>
