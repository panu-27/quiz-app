    ...
    <div className="min-h-screen bg-[#F6F8FC] pb-24">
      ...
      <div className="md:hidden flex flex-col pb-24">
          ...
          {rankLoading ? <TopRankMobileSkeleton /> : (
            <div className="relative bg-[#7A41F7] rounded-[2rem] p-5 flex items-center overflow-hidden cursor-pointer active:scale-[0.98] transition-transform" onClick={() => navigate("/student/personal")}>
              <div className="absolute right-0 bottom-0 opacity-20 pointer-events-none">
                <svg width="150" height="100" viewBox="0 0 150 100" fill="none"><circle cx="120" cy="80" r="60" stroke="white" strokeWidth="2" /><circle cx="120" cy="80" r="40" stroke="white" strokeWidth="2" /></svg>
              </div>
              ...
              <div className="absolute bottom-2 right-3 z-20 flex items-center justify-center w-10 h-10">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-white relative z-20" fill="currentColor" viewBox="0 0 24 24"><path d="M5 16L3 5L8.5 10L12 4L15.5 10L21 5L19 16H5M19 19C19 19.6 18.6 20 18 20H6C5.4 20 5 19.6 5 19V18H19V19Z" /></svg>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="px-4 mt-6">
          <div className="relative bg-[#7A41F7] rounded-3xl p-8 flex items-center min-h-[160px] overflow-hidden cursor-pointer active:scale-[0.98] transition-transform" onClick={() => navigate("/student/quiz")}>
            ...
            {quizzes.map((quiz, index) => (
              <div key={index} onClick={() => navigate(`/student/quiz?subj=${quiz.subj}`)} className={`${quiz.color} min-w-[260px] rounded-[2.5rem] p-7 flex-shrink-0 cursor-pointer transition-transform active:scale-95`}>
                ...
                <div className="flex items-center gap-8 text-[13px] font-bold text-slate-400">
                  <span>{quiz.chapters} Chapters</span><span>{quiz.pyq} Questions</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {PYQ_SUBJECTS.map((subj) => (
              ...
              >
                ...
                <div className="flex gap-2 mb-8">
                  ...
                  <div className={`${subj.badge} px-4 py-2 rounded-full text-[12px] font-bold text-slate-500/80 flex items-center gap-1`}>
        ...
        <div className="px-4 mt-10 pb-10">
          ...
          {testsLoading ? (
            <div className="space-y-4"><MobileTestRowSkeleton /><MobileTestRowSkeleton /></div>
          ) : tests.length > 0 ? (
            ...
            <div onClick={() => navigate("/student/history")} className="group relative bg-[#DDCEFD] p-4 rounded-[1.8rem] border border-[#F0F0F5] shadow-sm flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-[1.2rem] bg-[#7A41F7] flex items-center justify-center flex-shrink-0 shadow-md shadow-purple-200">
                  <div className="relative"><History className="text-white" size={24} /><div className="absolute -right-1 -top-1 w-3.5 h-3.5 bg-green-400 rounded-full border-2 border-[#7A41F7]" /></div>
                </div>
                <div>
          ...
          <div className="px-4 -mb-20">
            ...
            <div onClick={() => navigate("/student/history")} className="group relative bg-[#DDCEFD] p-4 rounded-[1.8rem] border border-[#F0F0F5] shadow-sm flex items-center justify-between cursor-pointer active:scale-[0.98] transition-all overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h5 className="text-[16px] font-bold text-[#1E1E2D] leading-tight">Quiz History</h5>
                  <p className="text-[13px] font-medium text-[#7A41F7]/70 mt-1">View detailed analysis • Performance</p>
                </div>
              </div>
              <div className="pr-2"><ChevronRight className="text-[#7A41F7] group-hover:translate-x-1 transition-transform" size={20} /></div>
      ...
      <div className="hidden md:block max-w-7xl mx-auto px-8 lg:px-8 xl:px-24 2xl:px-20 py-8">
        ...
        {rankLoading ? <HeroRowSkeleton /> : (
          <div className="grid grid-cols-3 gap-6 mb-8">
            <div className="col-span-2 relative bg-[#7A41F7] rounded-3xl p-10 flex items-center min-h-[220px] overflow-hidden cursor-pointer group" onClick={() => navigate("/student/quiz")}>
            ...
            <div className="relative bg-[#7A41F7] rounded-3xl p-6 flex flex-col justify-between overflow-hidden cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition-transform" onClick={() => navigate("/student/leaderboard")}>
              <div className="absolute right-0 bottom-0 opacity-20 pointer-events-none">
                <svg width="200" height="150" viewBox="0 0 200 150" fill="none"><circle cx="160" cy="120" r="90" stroke="white" strokeWidth="2" /><circle cx="160" cy="120" r="60" stroke="white" strokeWidth="2" /></svg>
              </div>
            ...
            {quizzes.map((quiz, index) => (
              <div key={index} onClick={() => navigate(`/student/quiz?subj=${quiz.subj}`)} className={`${quiz.color} rounded-3xl p-6 cursor-pointer hover:scale-[1.03] active:scale-95 transition-transform`}>
                ...
                <div className="flex flex-col gap-1 text-[12px] font-bold text-slate-400">
                  <span>{quiz.questions} Questions</span><span>{quiz.players} Players</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2">
            ...
            {testsLoading ? (
              ...
              <div className="space-y-3">
                {tests.map((t, idx) => (
                  <div key={t._id} className="flex items-center gap-3 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-purple-100 transition-all group">
                    <div className="w-8 h-8 rounded-xl bg-[#F3EBFF] flex items-center justify-center text-[#7A41F7] font-black text-sm flex-shrink-0">{String(idx + 1).padStart(2, "0")}</div>
                    <div className="w-11 h-11 rounded-xl bg-[#F8F7FF] flex items-center justify-center flex-shrink-0 border border-slate-100"><Clock className="text-[#7A41F7]" size={18} /></div>
                    ...
                    <button onClick={() => navigate(`/student/test/${t._id}`)} className="group/btn flex items-center gap-2 px-5 py-2.5 bg-[#7A41F7] hover:bg-[#6832E3] text-white rounded-xl text-sm font-bold transition-all active:scale-95 shadow-md shadow-purple-100 flex-shrink-0">
              ...
              <div className="flex flex-col items-center justify-center py-39 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                <div className="w-14 h-14 bg-slate-50 rounded-full flex items-center justify-center mb-3 shadow-sm"><ClipboardCheck className="text-slate-300" size={28} /></div>
                <p className="text-sm font-bold text-slate-400">No active tests scheduled</p>
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <div onClick={() => navigate("/student/history")} className="group relative bg-gradient-to-br from-[#7A41F7] to-[#9B6AF9] p-6 rounded-3xl shadow-lg shadow-purple-200/50 flex flex-col justify-between cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition-all overflow-hidden min-h-[170px]">
            ...
            {PYQ_SUBJECTS.map((subj) => (
              ...
              >
                ...
                <div className="flex gap-2 mb-6 flex-wrap">
                  ...
                  <div className={`${subj.badge} px-3 py-1.5 rounded-full text-[11px] font-bold text-slate-500/80 flex items-center gap-1`}>
